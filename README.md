...
Use `make stop` to stop the local Compose stack without deleting volumes, or `make reset` to stop it
and remove Compose volumes.
...
Backend tests cover state transitions, RBAC denial/audit logging, request submission, trace/correlation propagation, rate limiting, queued lifecycle worker execution, archive-retention sweeps, project ownership and scoped audit visibility, project member management/reassignment/suspension, approval information requests, CTO override, approval history, role-change and provisioning evidence visibility, provider adapter profiles, provider retryable failure handling, provider configuration modes, guarded live-provider operations, provider credential inventory/rotation, cancellation, extension requests, incidents, notifications, executive and adoption reporting, usage/cost/budget evidence, audit/cost allocation export, policy versioning/retention/evaluation, and mock provisioning through approval. Frontend tests cover request form validation.
