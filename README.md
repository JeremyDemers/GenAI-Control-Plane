...
- Next.js dashboard with request form, request cancellation, extension workflow, approvals, policy evaluation, policy version management, provider health/configuration visibility, usage and budget evidence, incident handling, notifications, CTO executive reporting, audit export, and spend charts.
...
Backend tests cover state transitions, RBAC denial/audit logging, request submission, cancellation, extension requests, incidents, notifications, executive reporting, usage/cost/budget evidence, audit export, policy versioning/evaluation, and mock provisioning through approval. Frontend tests cover request form validation.
