...
`OIDC_JWKS_URL` or `OIDC_JWKS_JSON`. Local development uses seeded identities passed through
`x-dev-user`. Frontend authorization-code flow with PKCE and secure refresh token handling remain the
next identity-layer step.
